<?php 
        ...
        @$publierComment = $_POST['publierComment'];

        $error = "";
        $valid = "";

        if(isset($publierComment)){
            require('./bdd/connexionBdd.php');

            $requete = $connexion -> prepare("UPDATE commentaires SET etatUser = 1 WHERE id_comment =:num LIMIT 1");
...
<style>
    ...
    <div id="container" data-aos="fade-up" data-aos-delay="100">
        <form method="POST" class="form formDelete">   
            <div>
                <label class="form-label mb-3">Voulez-vous vraiment publier ce commentaire ?</label>
                
                <p>
                    <input type="hidden" name="idComment" value="<?php echo $commentaire['id_comment']; ?>">
                    <button  name="publierComment" type="submit" class="btn btn-danger">Publier</button> 
